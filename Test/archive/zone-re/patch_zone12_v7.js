// Patch AutoCtrl_2025.4.22.0.exe v7 — SELECTIVE zone-12 classifier fix (take 2).
//
// Live facts (Chrome 150 SxS, tree ON, probe6):
//   over a TAB:      hovered role 41 (0x29), parent role 37 (0x25)
//   over strip gap:  hovered role 16 (0x10), parent role 60 (0x3c)
//   over omnibox:    role 42 (0x2a) — zone 21 unaffected
//   over page:       role 15 (0x0f)  — zone 3 unaffected
//
// v6 failure: I changed the PARENT cmp 0x25->0x29 — but the parent of a tab
// IS role 37 (0x25), so the parent check never matched. v7 keeps the parent
// cmp at 0x25 and instead widens the HOVERED role check via a stub in the
// .acp section: hovered ∈ {0x25, 0x29, 0x3c, 0x10} -> zone 12 true.
//
// Also the FUN_00414760 gate (jz fail after the zone-12 window check) is
// NOPed — v5b proved the gate returns 0 over the tab strip on Chrome 150
// (its monitor-scaled rect cache is broken for the modern maximized window).
//
// Changes:
//   1) 0x415781: `0f 84 69 03 00 00` (jz 0x415af0) -> 6x NOP  [gate bypass]
//   2) 0x4155ac: `83 7e 2c 25 75 08` (cmp [esi+0x2c],0x25; jnz +8)
//      -> `e9 rel32; 90` jmp into the .acp stub
//   3) .acp stub: hovered role ∈ {0x25,0x29,0x3c,0x10} -> return 1
//      (jmp back to 0x4155b2, the mov al,1 epilogue); else fall into the
//      ORIGINAL parent path at 0x4155ba (parent cmp stays 0x25).
'use strict';
const fs = require('fs');
...
// ---- add the .acp section (same machinery as patch_accname.js) ----
const IMG = 0x400000;
const last = secs[secs.length - 1];
const fileAlign = u32(pe + 24 + 36), sectAlign = u32(pe + 24 + 32);
const newRaw = Math.ceil((last.raw + last.rsize) / fileAlign) * fileAlign;
...
// ---- 2) hovered-role cmp -> jmp into stub ----
...
let je1 = stub.length; w8(0x74, 0x00);       // je PASS
