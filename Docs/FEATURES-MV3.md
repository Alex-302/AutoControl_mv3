...
| 17 | Open URL `chrome://history` / bookmarks stuck on Loading or blank (gestures) | ✅ | **fixed 2026-09-11** — same Chrome WebUI-from-SW paint class as sessions.restore, **different API** (do NOT fold into that PR). Open URL (`_Mh` file8) → `_6a`/`_3g` (file34) → `chrome.tabs.create` / `windows.create` with `active:false`. Gesture vs hotkey use the SAME action runner; gestures additionally schedule RBTN-ESC (type 300 Esc, 20ms). `chrome://history` from a SW create often sticks on "正在加载…"; bookmarks sometimes blank; F5 always heals. Keyboard Alt+X of the same URL works (no Esc). FIX: wrap `chrome.tabs.create` + `chrome.windows.create` in sw.js after importScripts; after create, if the URL is `chrome://` or `edge://` (skip `about:blank` and the new-tab page), `chrome.tabs.reload` once immediately. Gesture 750s that Open URL chrome:// are held ~35ms so RBTN-ESC (20ms) lands on the old tab first (Esc on the new WebUI → Loading… then blank). Other gestures are not delayed. https Open URL is not reloaded (would flash). `_Yk === chrome` in the SW so `_6a`/`_3g` pick up the wrap; page-proxy `tabsCreate`/`winsCreate` too. NO bundle rebuild. mh_test B55. |
