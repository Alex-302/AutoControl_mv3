...
> (`../ext-mv2/unpacked/`, the tree extracted from the original Web Store
> crx in `ext-mv2/`) against `../mv3-build/`, plus official Chrome platform docs
...
| Hover-sensitive regions | ✅ | **SOLVED 2026-09-12** — the ORIGINAL engine cannot classify the browser chrome on Chrome 148+ (MSAA tree activation + a hover cache that never refreshes over the tabs; RE report in `archive/NATIVE-REVERSING-2026-08-31.md`). Fix: patched engine v19 (the region check is a table lookup) + an external zone helper; ALL 12 regions verified live — see §7-12 and `Docs/TODO-mouseover-zones.md`. Requires the patched engine (installed by `Test/deploy_patched_engine.ps1`) |
| Clipboard change | ✅ | |
...
| Feature | MV3 | Notes |
|---|---|---|
| All trigger/action editors | ✅ | file68/file72 byte-identical; the temporary "⚠ broken in Chrome 148+" marks on the five region options were REMOVED 2026-09-13 — the zones work with the patched engine |
...
| 12 | Hover regions on Chrome 148+ | ✅ | **SOLVED 2026-09-03, COMPLETE 2026-09-12 — ALL 12 action-editor areas VERIFIED by the user on a live browser** (Browser window · Web page · Title area · Browser tab · Tab's close button · Tab's speaker icon · New tab button · Toolbar · Omnibox · Bookmark button · Browser menu button · Any menu item; each tested with a physical wheel, evidence in the SW log `zones=[…] ∩ […] → executing`, negative controls silent — `Docs/TODO-mouseover-zones.md`) — root cause (2026-08-31): MSAA tree activation (honey pot + accName — the engine never queries accName) + the engine's hover cache never refreshes over the tab strip + fresh MSAA from the LL-hook DEADLOCKS (RE doc §12). FIX: (1) patched engine **v19** (`1A10EDD1…`, `AutoControl_native/patches/patch_zones_v19.js`, SUPERSEDES v18 — **2026-09-12 later that day**): v18 (every region below 60 matches) turned out to also break INPUT CONSUMPTION — the matcher's return value IS the "consume the input" decision, so a wheel trigger with any mouse-over condition ate the wheel everywhere and pages stopped scrolling. v19 moves the decision into a **zone table** the helper writes into the engine's memory (`alive` flag + 64 slots, one per region): the cave trampoline at `FUN_004156f0` reads `table[region]` and matches only when the helper reports the cursor over that region; regions ≥ 40 keep the engine's own logic and a missing helper (alive=0) falls back to the v18 behaviour. VERIFIED LIVE with real OS input: page wheel scrolls (scrollY 800→1100→2100, zero false 750s) with the zone-12 trigger enabled, wheel over the tab strip fires it (`zones=[12,4,1] ∩ [12] → executing`). ⚠ **CORRECTION (2026-09-12, user-verified): regions ≥ 21 and the menu items DO work** — omnibox (`[21,20,4,1] ∩ [21]`), bookmark star (`[33,20,4,1] ∩ [33]`), browser-menu button (`[30,20,4,1] ∩ [30]`) and "Any menu item" (region 40, the engine's own verdict — fired over the OPEN menu only, silent over the page/tab strip/beside it). The earlier "unsupported" note was measured while the engine's MOUSE hook was dead after a forced restart (a dead mouse hook looks exactly like "this zone is not detected" — always re-run a known-good control first); (2) external zone helper `ac_zone_helper.exe` (native host `com.autocontrol.zonehelper`, source `Test/ac_zone_helper.cs`, no hooks) classifies the cursor region via MSAA from a normal process; (3) sw.js `__acDispatchTrigger750` gate dispatches mouseOver-gated triggers ONLY when the trigger's regions intersect the helper's SET. (2) external zone helper `ac_zone_helper.exe` (native host `com.autocontrol.zonehelper`, source `Test/ac_zone_helper.cs`, no hooks) classifies the cursor region via MSAA from a normal process; (3) sw.js `__acDispatchTrigger750` gate dispatches mouseOver-gated triggers ONLY when the trigger's regions intersect the helper's SET. Supported 2026-09-12: **1** window · **3** page (role 15) · **4** title area · **12** tab (role 37 / strip gap 60) · **15** close (43 under a PAGETAB, right half) · **16** new-tab "+" (43 under the PAGETABLIST) · **17** speaker (43 under a PAGETAB, left half) · **20** toolbar (role 22 in the ancestry) · **21** omnibox (42, or the lock 57 under the GROUPING) · **30** menu button (square button hugging the window's right edge — the kebab is not exposed in Chrome 150) · **33** bookmark star (43 inside the GROUPING in the toolbar). NOT implemented in the helper: menu items 40-51 (the native tracks the hovered item — type 185 — so the SW-side path is the cheaper one) — since 2026-09-12 the gate PASSES such triggers through to the engine's own verdict (`__AC_ZONE_KNOWN` filter; before that every menu-item trigger was silently skipped). Also 2026-09-12: all 750s of one input burst share ONE helper answer (`__AC_ZONE_CACHE_MS = 120`) — Chrome can scroll the tab strip between them, so the burst used to see different regions for one physical wheel ("wheel over the close button does nothing"). Verified live with real OS wheel through the native hook: `trig 41: zones=[12,1] ∩ [12] → executing` (tab, control), `trig 92: zones=[15,1] ∩ [15] → executing` (close button ✅), `trig 96: zones=[16,1] ∩ [16] → executing` ("+" ✅), `trig 95: zones=[30,20,1] ∩ [30] → executing` (update pill = menu button ✅), `zones=[3,1] ∉ … → skipped` (page); per-point helper answers for all supported regions in the TODO §2d. ⚠ A browser RESTART makes the extension import `settings.dat` into storage (MV2 startup behaviour) — live test triggers are replaced by the file's content. ⚠ Deployment: the helper exe lives in `%LOCALAPPDATA%\AutoControl\` (reload the extension after rebuilding it); if Chrome loads this extension from a COPY of the repo folder, `sw.js` must be updated in that copy too (a stale copy silently runs the old gate) |
