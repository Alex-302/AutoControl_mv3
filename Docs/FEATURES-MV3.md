...
| Hover-sensitive regions | ⚠️ | regions "Browser tab", "Tab's close button", "Tab's speaker icon", "New tab button", "Any menu item" **broken in Chrome 148+** (native a11y hit-test) — affects MV2 and MV3 equally, NOT a port loss; UI marks them. Title area / Web page / Omnibox work |
| Clipboard change | ✅ | |
...
| 12 | Hover regions "Browser tab", "close button", "speaker icon", "new tab", "menu item" in Chrome 148+ | ⚠️ | 🔒 native-side (a11y hit-test), same in MV2 — NOT tracked (§8) |
